...
function even_odd(num1:number):string{
    if(num1%2==0)
        return "even";
    else
        return "odd";
...
function fahrenheit(num1:number):number{
    let f=(9/5)*num1+32; 
    return f;
}
...
console.log("Tempereture in fahrenheit "+fahrenheit(answers.num1));
*/
...
myArray.push(100);
console.log("After push:", myArray); // Output: [2, 4, 6, 8, 10,100]

// pop: Remove the last element from the array
let popN = myArray.pop();
console.log("After pop:", myArray); // Output: [2, 4, 6, 8, 10]
console.log("Popped Element:", popN); // Pop Value: 100

// shift: Remove the first element from the array
let shift = myArray.shift();
console.log("After shift:",  myArray); // Output:[4, 6, 8, 10]
console.log("Shifted Element:", shift); // Shift value: 2
...
console.log("pop Element:", pop); // poped value: haider

// shift: Remove the first element from the  string array
let shiftS = str.shift();
console.log("After shift:", str); // // output: [usman,ali]
console.log("Shifted Element:", shiftS); // Shifted value: waqas

let unshiftS = str.unshift('Naveed','Waqas','Zaheer');
console.log("After unshift method :", str);

//Scenario 2 - Subarray Creation:

// Initialize an array with some initial elements
let originalArray = [12,24,36,48,60,72,84];

// splice: Create a subarray by removing elements from the originalSubarray 
let remElement = originalArray.splice(2, 3); // Remove 3 elements starting from index 2
console.log("After splice:", originalArray); // Output: [12,24,72,84]
console.log("Removed Elements:", remElement); // Output: [36, 48, 60]

// slice: Create a subarray without modifying the original array
let subArray = originalArray.slice(1,3); // Create a subarray from index 1 to 2 (3 is exclusive)
console.log("Original subArray:", originalArray); // Output:  [ 12, 24, 72, 84 ]
